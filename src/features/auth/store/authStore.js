...
export const useAuthStore = create((set) => ({
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      if (role === 'doctor') {
        await supabase.from('doctor_availability').upsert({ doctor_id: data.user.id, current_status: 'Available Now' }, { onConflict: 'doctor_id' });
      }
  ...
  logout: async () => {
    ...
    if (state.role === 'doctor' && state.user?.id) {
      await supabase.from('doctor_availability').upsert({ doctor_id: state.user.id, current_status: 'Offline' }, { onConflict: 'doctor_id' });
    }
